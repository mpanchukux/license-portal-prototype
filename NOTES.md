...
**Хром**
- **Топбар — один ряд, і в ньому лише три речі**: **бургер + мітка лого** ліворуч,
  **user-іконка** праворуч (`"burger brand act prof"`, де `act` порожній, доки не
  скролять). Бенд лишається contained (`--pageX:16px`) і зі скругленим низом.
  **Текст лого схований** (`.dbrand .bt`).
...
- ⚠️ **Скрол-смугу nav замінено на бургер (2026-08-31).** П'ять пунктів, що скролились
  боком, читались як три: «Users» був за кадром. Тепер **`.navburger`** (44×44)
  відкриває **`.navsheet`** — панель на всю висоту з лівого краю (`min(320px,86vw)`)
  над бекдропом: «Menu» + ✕, під ним ті самі п'ять пунктів по 48px. `.tnav` лишається
  в DOM (це десктопний nav) і просто схований. **Одне джерело пунктів** —
  `navItemsHTML(extraClass)` рендерить і бар, і панель; пункти панелі несуть той самий
  клас `.tnav-item` і `data-nav`, тому поточну сторінку в них позначає **той самий
  `syncTopNav`** (`.on` + `aria-current="page"`). Закривають ✕, бекдроп, Esc і клік по
  пункту; `body.navsheet-open` блокує скрол. `revealActiveNav()` видалено як мертве.
...
- **Таблиці стають картками рядків** — жодного горизонтального скролу для головних
  ...
  **Ліцензія — дві частини, розділені простором** (перероблено 2026-08-31; **хейрлайнів
  усередині картки немає**): (1) квадрат + `продукт · тип` ліворуч і **статус-чіп
  праворуч на тій самій лінії**, під ними мітка, якщо є; (2) **назва плану ліворуч +
  рядок стану праворуч на одній лінії**; (3) дії. Розділяє `row-gap:10px`, не лінії.
  ⚠️ **`display:contents` на клітинці статусу і на `.licstat`** — саме воно піднімає
  чіп і рядок стану в грід самої картки, і лише тому кожен може стояти у своєму рядку
  навпроти свого підмета.
  ⚠️ **Plan і стан ділять рядок 2 на обидві колонки** (`grid-row:2; grid-column:1/-1`
  + `justify-self`), а не беруть колонку кожен: трек, зміряний по «Renews Sep 02, 2026»
  (~134px), крав ширину в рядка вище й ламав `ThingsBoard · Subscription` на два рядки.
  Тепер `auto`-колонку міряє **лише чіп** (57px) — заміряно: тип-рядок в один рядок у
  всіх п'яти картках, колонки 262/57, картки 138–173px.
  ⚠️ Прибрано мертве `tr.lic-row > td + td{margin-top:8px}` з першого пасу карток: воно
  вигравало за специфічністю й додавало 8px, через що план і стан не сходились на одну
  лінію (заміряно 338 проти 330).
  ⚠️ **Колонки Updated у картці немає**: у трьох замовлених бендах місця для неї не
  було, тому на телефоні вона `display:none` (у десктопній таблиці лишається).
  Дії більше **не absolute** — вони третій бенд у потоці, тож резерв `96px` праворуч
  знято. Заміряно: 202px із міткою, 188px без.
  **Інвойс — два бенди**: (1) номер ліворуч + `[іконка авто-списання] статус-чіп`
  праворуч (порядок дає `flex-direction:row-reverse` на `.statwrap` — у DOM чіп перший);
  (2) рядок `продукт · тип`, під ним сума ліворуч + дата праворуч, під ними дії.
  Дільник — `border-top` клітинки продукту. ⚠️ У табі Invoices самої ліцензії
  продукт-клітинки немає (`noProduct`), тому дільник переїжджає на рядок суми через
  `tr.inv-row:not(:has(.lic-prodcell)) > td.num`.
  **Дії інвойса стали icon-buttons** (44×44): download і view ліворуч, **open-license
  праворуч** (`margin-left:auto`). Для цього `invRow` віддає в кожній дії **і гліф, і
  лейбл** — десктоп читає слова (`.ra-ic{display:none}`), телефон читає іконки
  (`.ra-txt{display:none}`). Кнопка open-license (`.ra-open`, `.mob-only`) існує
  **лише на телефоні**: на десктопі цю роль виконує Product-клітинка, яка й так лінк.
  На табі самої ліцензії вона схована — вела б на ту саму ліцензію. Заміряно: 171px.
...
- **Full-screen sheet**: 390×844, `border-radius:0`, без рамки й бекдроп-полів.
  Хедер `position:sticky` (титул ліворуч, ✕ праворуч), контент скролиться під ним.
- ⚠️ **Порядок хедера на телефоні — титул → мітка → дії** (2026-08-31).
  `.headactions` лежить усередині `.top`, тому `.top` розчиняється
  (**`display:contents`**), її діти потрапляють у флекс-колонку `.headcol`, і лише тоді
  `order` може поставити мітку між титулом і діями (`.idline` 1 · `.metarow` 2 ·
  `.headactions` 3, full-width). **Десктоп не змінювався**: `.headcol` лишається
  блоком, `.top` — флексом, дії притиснуті праворуч на рядку титулу (перевірено на 1280).
- ⚠️ **Відступ контенту не належить `.fs-body`**: `.licmodal .fs-body{padding:0}`
  специфічніший за `.fs-body{padding:16px}` у медіа-квері, тож інсет тримають `.head`
  і `.section` — обидва зведені на 16px. Заміряно: **усі ліві кромки на 16px, розкид 0**
  (титул, кікер, лейбл ключа, значення, «Plan & add-ons», перший рядок таблиці, таби).
- Ключ — **18px**: розкритий (29 символів) лягає одним рядком у 317px і не переноситься
  посередині значення. Іконки лишаються поруч, а якщо не влізуть — переносяться рядком.
- Plan & add-ons — стековані рядки: назва на всю ширину, під нею
  `INCLUDED / EXTRA / LIMIT` як пари лейбл-значення (лейбли через `::before`).
- Таби — скрол-смуга; Next charge стекується (сума+дата, під ними метод, олівець
  праворуч на тому ж рядку); хедер лишає **лише праймері**, а Apply coupon переїжджає
  в ⋮ (`.mob-only`, натискає справжню кнопку, тож контролер купона один).
...
- **`license.html` (режим Full page)** — не в скоупі. Він тримає канавку back-кнопки,
  тому контентна колонка вужча, і три іконки ключа розпадаються 1+2. Не ламається,
  але й не оптимізоване: дефолт — модалка.
...
- **Мобільна адаптація ≤600px** (Home + модалка деталей + хром): бургер-панель ·
  картки рядків замість таблиць · full-screen sheet · стековані Plan & add-ons,
  Instances, Next charge · пейджер = лічильник + крок · scroll-to-top FAB.
