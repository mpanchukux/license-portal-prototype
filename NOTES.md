...
`licDetailsMode()` у `shared.js`. **Дефолт — модалка**: `licDetailsMode()` віддає
`page`, лише якщо в сторі стоїть явний вибір `page`, тож і чистий стор, і старий (де
ключа ще немає) відкривають модалку. Контент і функціональність в обох режимах
